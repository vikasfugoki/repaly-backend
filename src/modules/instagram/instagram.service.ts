...
export class InstagramAccountService {
  ...
  async updateMediaResponseTypeOnTable(
    ...
    try {
      ...
      response['id'] = mediaId;
      return await this.instagramMediaRepositoryService.updateMediaDetails(
        response,
      );
    } catch (error) {
  ...
  async getRecentMedia(accountId: string, access_token: string) {
    try {
      ...
      console.error(
        `Failed to fetch recent media for accountId ${accountId}:`,
  ...
  async getInstagramRecentMedia(accountId: string) {
    ...
    console.log(account);
    if (!account) throw new Error('Token not found');
    const mediaWithInsights = await this.getRecentMedia(
      accountId,
    ...
    try {
      await this.instagramMediaRepositoryService.insertMultipleMediaDetails(
        mediaWithInsights,
      );
    } catch (error) {
  ...
  async addInstagramStoryAutomation(
    storyId: string,
    ...
    try {
      ...
      input['id'] = storyId;
      return await this.instagramStoryRepositoryService.updateStoryDetails(
        input,
      );
    } catch (error) {
      console.error(
        `Error inserting automation details for ${storyId}:`,
        error,
      );
      throw error;
    }
  ...
  async addInstagramMediaAutomation(
    mediaId: string,
    input: Record<string, any>,
  ) {
    try {
  ...
  async updateAccountMediaOnTable(accountId: string) {
    try {
      ...
      for (const media of mediaWithInsights) {
        await this.instagramMediaAnalyticsRepositoryService.updateAnalyticsDetails(
          media,
        );
      }
  ...
  async mediaStatsFromTable(mediaId: string) {
    try {
      ...
      return {
        success: false,
        message: `Error fetching media stats from table`,
      };
    }
  }
  ...
  async getCommentsFromAnalyticsTable(mediaId: string, type: string) {
    try {
      ...
      return {
        success: false,
        message: `Error fetching ${type} comments for media`,
      };
    }
  }
  ...
  async getCommentTimeSeriesByIntervals(mediaId: string) {
    try {
      ...
      const aggregateByInterval = (
        ...
        for (const type in commentTimeseries) {
          ...
          for (const entry of entries) {
            ...
            bucketCounts[bucketTs][type] =
              (bucketCounts[bucketTs][type] ?? 0) + entry.count;
          }
        }
      ...
      console.error(
        `Failed to fetch comment timeseries for media ${mediaId}:`,
        error,
      );
      return {
        success: false,
        message: 'Error fetching comment timeseries data',
      };
    }
  }
  ...
  async getRecentStory(accountId: string, access_token: string) {
    try {
      ...
      const storyList = await this.instagramApiService.getStories(
        accountId,
        access_token,
      );
      ...
      console.error(
        `Failed to fetch recent stroy for accountId ${accountId}:`,
        (error as Error).message,
      );
  ...
  async updateAccountStoryOnTable(accountId: string) {
    try {
      ...
      const storyDetails = await this.getRecentStory(
        accountId,
        account.access_token,
      );
      ...
      for (const story of storyDetails) {
        ...
        const hoursDiff =
          (now.getTime() - storyTimestamp.getTime()) / (1000 * 60 * 60);
        ...
        const response = await this.instagramStoryRepositoryService.getStory(
          story.id,
        );
        ...
        const storyWithExtras = {
          ...
          tag_and_value_pair:
            existingStory && 'tag_and_value_pair' in existingStory
      ...
      return {
        success: false,
        message: `Error updating story in the table ${error}`,
      };
    }
  }
  ...
  async getInstagramStoryFromTable(accountId: string) {
    try {
      ...
      const response =
        await this.instagramStoryRepositoryService.getStoryByAccountId(
          accountId,
        );
        ...
        items
          ...
          .map(async (item) => {
            ...
            if (item.IsActive !== isActive) {
              ...
              console.log(
                `Updated IsActive for story ${item.id} → ${isActive}`,
              );
            }
  ...
  async getStoryDetailsFromTable(storyId: string) {
    try {
      const response =
        await this.instagramStoryRepositoryService.getStory(storyId);
      const item = response?.Item ?? {};
    ...
    try {
      ...
      if (isActive) {
        const analyticsResponse =
          await this.instagramStoryAnalyticsRepositoryService.getStoryAnalytics(
            storyId,
          );
  ...
  async getDMCategorization(accountId: string) {
    try {
      const conversations =
        await this.instagramDMService.getConversationsByAccountId(accountId);
  ...
  async getDMConversation(conversationId: string) {
    try {
      const conversation =
        await this.instagramDMService.getConversations(conversationId);
  ...
  async updateDMCategory(
    ...
    try {
      ...
      console.error(
        `Failed to PUT conversation category for ${conversationId}:`,
        error,
      );
      throw error;
    }
  ...
  async getSummaries(mediaId: string) {
    try {
      ...
      const negativeSummary =
        mediaItem.negative_summary || 'No negative summary available.';
      const inquirySummary =
        mediaItem.inquiry_summary || 'No inquiry summary available.';
  ...
  async updateAdsOnTable(accountId: string, input: Record<string, any>) {
    try {
      const isAccountIdPresent =
        await this.instagramFbAccessTokenService.isIdPresent(accountId);
      ...
      if (!isAccountIdPresent) {
        ...
        if (!input?.code) {
          throw new BadRequestException(
            'Missing authorization code for new account',
          );
        }

        const tokenResponse = await this.facebookApiService.getAccessTokenAds(
          input.code,
        );
        ...
        const facebookItem =
          await this.instagramFbAccessTokenService.getFacebookDetails(
            accountId,
          );
        ...
        if (!accessToken) {
          throw new InternalServerErrorException(
            'Access token not found for existing account',
          );
        }
      }
      ...
      for (const ad of enrichedAdsList) {
        const adsWithData =
          await this.facebookApiService.getAdsWithCreativesAndInsights(
            ad.id,
            accessToken,
          );
        ...
        for (const adData of adsWithData) {
          ...
          if (igUserId && igUserId !== proUserId) {
            console.warn(
              `Skipping ad ${adData.ad_id} - Instagram user mismatch (${igUserId} ≠ ${proUserId})`,
            );
          ...
          if (adData?.effective_instagram_media_id) {
            ...
            await this.instagramAdAnalyticsRepositoryService.updateAdAnalytics(
              adAnalytics,
            );
          }
      ...
      return {
        success: true,
        message: 'No instagram Ads available in this account!!!',
      };
    } catch (error) {
  ...
  async getAdsFromTable(accountId: string) {
    try {
      ...
      console.error(
        `Failed to get ad details for account: ${accountId}`,
        error,
      );
      throw error;
    }
  ...
  async isFacebookLinked(accountId: string) {
    try {
      ...
      console.error(
        `Failed to facebook account linked status to instagram account: ${accountId}`,
        error,
      );
      throw error;
    }
  ...
  async getAdStatsFromTable(adId: string) {
    try {
      const result =
        await this.instagramAdAnalyticsRepositoryService.getAdAnalytics(adId);
      ...
      console.error(
        `Error getting ad analytic stats details for ${adId}:`,
        error,
      );
      throw error;
    }
  ...
  async getCommentsFromAdAnalyticsTable(
    adId: string,
    type: 'positive' | 'negative' | 'potential_buyers' | 'inquiry',
  ) {
    try {
      const result =
        await this.instagramAdAnalyticsRepositoryService.getAdAnalytics(adId);
      ...
      return {
        success: false,
        message: `Error fetching ${type} comments for ad`,
      };
    }
  }
  ...
  async dmReply(
    ...
    try {
      ...
      if (!igId) {
        throw new Error(
          'Instagram ID (pro_user_id) is missing for this account',
        );
      }
      ...
      const response = await fetch(
        `https://graph.instagram.com/v23.0/${igId}/messages`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${access_token}`,
      ...
      console.error(
        `Failed to send message for account ${accountId}:`,
        error?.message || error,
      );
      return {
        success: false,
  ...
  async getAccountLevelAnalytics(accountId: string) {
    try {
      ...
      await Promise.allSettled([
        ...
        results.forEach((result, idx) => {
          if (result.status === 'rejected') {
            const fn = [
              'updateAccountMediaOnTable',
              'updateAdsOnTable',
              'updateAccountStoryOnTable',
            ][idx];
      ...
      const stats =
        await this.instagramAccountLevelAnalyticsRepositoryService.getAccountLevelAnalyticsByAccountId(
          accountId,
        );
      const accountInfo =
        await this.instagramAccountRepositoryService.getAccount(accountId);
      ...
      const media_count = await this.instagramApiService.getMediaCount(
        accountId,
        access_token,
      );
      console.log('access token:', access_token);
      ...
      const levels = [
        'account_media',
        'account_ads',
        'account_media_automated_posts',
        'account_ad_automated_posts',
        'account_dm_automated_posts',
      ];
      ...
      const mediaDetails = {
        ...
        replied_comments:
          ...
          (media.positive || 0) +
          (media.negative || 0) +
          (media.potential_buyers || 0) +
          (media.tagged_comment || 0) +
          (media.tagged_comment_dm || 0) +
          (media.inquiry_dm || 0),
        buyers:
          (media.potential_buyers || 0) +
          (media.potential_buyers_no_automation || 0),
        inquiries:
          (media.inquiry || 0) +
          (media.inquiry_no_automation || 0) +
          (media.inquiry_dm || 0),
        negative_comments:
          (media.negative || 0) + (media.negative_no_automation || 0),
        positive_comments:
          (media.positive || 0) + (media.positive_no_automation || 0),
        others: media.other_comments || 0,
        tagged_comment:
          (media.tagged_comment || 0) + (media.tagged_comment_dm || 0),
      ...
      const adsDetails = {
        ...
        replied_comments:
          ...
          (ads.positive || 0) +
          (ads.negative || 0) +
          (ads.potential_buyers || 0) +
          (ads.tagged_comment || 0) +
          (ads.tagged_comment_dm || 0),
        ...
        others:
          (ads.others || 0) +
          (ads.positive_no_automation || 0) +
          (ads.negative_no_automation || 0) +
          (ads.potential_buyers_no_automation || 0) +
          (ads.inquiry_no_automation || 0),
        tagged_comment:
          (ads.tagged_comment || 0) + (ads.tagged_comment_dm || 0),
      ...
      console.error(
        `Failed to fetch account analytics for ${accountId}:`,
        error,
      );
  ...
  async getDMSummaryAnalytics(accountId: string) {
    try {
      ...
      try {
        ...
        console.error(
          `updateAccountStoryOnTable failed for ${accountId}:`,
          err,
        );
      }
      ...
      const stats =
        await this.instagramAccountLevelAnalyticsRepositoryService.getAccountLevelAnalytics(
          accountId + '_dm_automated_posts',
        );
      ...
      if (stats.Item.total_dm === undefined || stats.Item.total_dm === null) {
        ...
        stats.Item.total_dm =
          ...
          category_lead;
        console.log(
          `Calculated total_dm: ${stats.Item.total_dm} from categories`,
        );
      }

      // If total_dm is 0 but total_unreplied exists, use total_unreplied as total_dm
      if (
        stats.Item.total_dm === 0 &&
        stats.Item.total_unreplied !== undefined &&
        stats.Item.total_unreplied !== null
      ) {
        stats.Item.total_dm = stats.Item.total_unreplied;
        console.log(
          `Using total_unreplied (${stats.Item.total_unreplied}) as total_dm since calculated total_dm was 0`,
        );
      }
  ...
  async getDMConversations(accountId: string) {
    try {
      ...
      const { items, lastEvaluatedKey } =
        await this.instagramDmMessageDetailsService.getConversationDetails(
          accountId,
        );
  ...
  async getDMConversationDetails(account_id: string, conversationId: string) {
    try {
      ...
      const conversation =
        await this.instagramDmMessagesService.getConversationById(
          conversationId,
        );
      ...
      console.error(
        `Failed to get DM conversation details for ${conversationId}:`,
        error,
      );
  ...
  async getMediaAnalytics(accountId: string) {
    try {
      ...
      try {
        ...
        console.error(
          `updateAccountMediaOnTable failed for ${accountId}:`,
          err,
        );
      }

      const mediaAutomatedPostsResult =
        await this.instagramAccountLevelAnalyticsRepositoryService.getAccountLevelAnalytics(
          accountId + '_media_automated_posts',
        );
      const mediaAutomatedPosts = mediaAutomatedPostsResult?.Item || {};
      const mediaAnalyticsResult =
        await this.instagramAccountLevelAnalyticsRepositoryService.getAccountLevelAnalytics(
          accountId + '_media',
        );
      ...
      const res = {
        ...
        no_automation_comments: mediaAnalytics.no_automation_comments ?? 0,
        other_comments: mediaAnalytics.other_comments ?? 0,
        positive: mediaAnalytics.positive ?? 0,
        positive_no_automation: mediaAnalytics.positive_no_automation ?? 0,
        potential_buyers: mediaAnalytics.potential_buyers ?? 0,
        potential_buyers_no_automation:
          mediaAnalytics.potential_buyers_no_automation ?? 0,
        tagged: mediaAnalytics.tagged ?? 0,
        tagged_comment: mediaAnalytics.tagged_comment ?? 0,
        tagged_comment_dm: mediaAnalytics.tagged_comment_dm ?? 0,
        total_comments: mediaAnalytics.total_comments ?? 0,
        total_dms: mediaAnalytics.total_dms ?? 0,
        media_automated_posts: mediaAutomatedPosts.automated_post ?? 0,
        total_posts: mediaAutomatedPosts.total_post ?? 0,
        total_positive:
          (mediaAnalytics.positive ?? 0) +
          (mediaAnalytics.positive_no_automation ?? 0),
        total_negative:
          (mediaAnalytics.negative ?? 0) +
          (mediaAnalytics.negative_no_automation ?? 0),
        total_potential_buyers:
          (mediaAnalytics.potential_buyers ?? 0) +
          (mediaAnalytics.potential_buyers_no_automation ?? 0),
        total_inquiry:
          (mediaAnalytics.inquiry ?? 0) +
          (mediaAnalytics.inquiry_no_automation ?? 0) +
          (mediaAnalytics.inquiry_dm ?? 0),
        total_tagged:
          (mediaAnalytics.tagged_comment ?? 0) +
          (mediaAnalytics.tagged_comment_dm ?? 0),
  ...
  async getMediaCommentsByCategory(
    ...
    try {
      ...
      if (category === 'positive') {
        ...
        processComments(
          commentsByType['positive_no_automation'] || [],
          'positive_no_automation',
        );
      } else if (category === 'negative') {
        processComments(commentsByType['negative'] || [], 'negative');
        processComments(
          commentsByType['negative_no_automation'] || [],
          'negative_no_automation',
        );
      } else if (category === 'potential_buyers') {
        processComments(
          commentsByType['potential_buyers'] || [],
          'potential_buyers',
        );
        processComments(
          commentsByType['potential_buyers_no_automation'] || [],
          'potential_buyers_no_automation',
        );
        ...
        processComments(
          commentsByType['inquiry_no_automation'] || [],
          'inquiry_no_automation',
        );
      } else if (category === 'tagged_comment') {
        processComments(
          commentsByType['tagged_comment'] || [],
          'tagged_comment',
        );
        processComments(
          commentsByType['tagged_comment_dm'] || [],
          'tagged_comment_dm',
        );
      } else if (category === 'others') {
        processComments(commentsByType['others'] || [], 'others');
        processComments(
          commentsByType['other_comments'] || [],
          'other_comments',
        );
      }
      ...
      console.error(
        `Failed to get ${category} comments for media ${mediaId}:`,
        error,
      );
  ...
  async getMediaCommentCounts(accountId: string) {
    try {
      ...
      console.error(
        `Failed to get media comment counts for account ${accountId}:`,
        error,
      );
      throw error;
    }
  ...
  async getAdsAnalytics(accountId: string) {
    try {
      ...
      const adsAutomatedPostsResult =
        await this.instagramAccountLevelAnalyticsRepositoryService.getAccountLevelAnalytics(
          accountId + '_ad_automated_posts',
        );
      const adsAutomatedPosts = adsAutomatedPostsResult?.Item || {};
      const adsAnalyticsResult =
        await this.instagramAccountLevelAnalyticsRepositoryService.getAccountLevelAnalytics(
          accountId + '_ads',
        );
      ...
      const res = {
        ...
        potential_buyers_no_automation:
          adsAnalytics.potential_buyers_no_automation ?? 0,
        ...
        total_positive:
          (adsAnalytics.positive ?? 0) +
          (adsAnalytics.positive_no_automation ?? 0),
        total_negative:
          (adsAnalytics.negative ?? 0) +
          (adsAnalytics.negative_no_automation ?? 0),
        total_potential_buyers:
          (adsAnalytics.potential_buyers ?? 0) +
          (adsAnalytics.potential_buyers_no_automation ?? 0),
        total_inquiry:
          (adsAnalytics.inquiry ?? 0) +
          (adsAnalytics.inquiry_dm ?? 0) +
          (adsAnalytics.inquiry_no_automation ?? 0),
        total_tagged:
          (adsAnalytics.tagged_comment ?? 0) +
          (adsAnalytics.tagged_comment_dm ?? 0),
      ...
      for (const [rawCategory, comments] of Object.entries(commentsByType)) {
        ...
        for (const c of comments as any[]) {
          allComments.push({
            ...
            response_comment: rawCategory.includes('dm')
              ? ''
              : c.comment_response,
  ...
  async getAdCommentCounts(accountId: string) {
    try {
      ...
      const adsListResponse =
        await this.instagramAdsService.getAdsByAccountId(accountId);
      ...
      const result = await Promise.all(
        adsItems.map(async (ad) => {
          ...
          const analytics =
            await this.instagramAdAnalyticsRepositoryService.getAdAnalytics(
              adId,
            );
      ...
      console.error(
        `Failed to get ad comment counts for account ${accountId}:`,
        error,
      );
      throw error;
    }
  ...
  async getQuickReplies(accountId: string) {
    try {
      const quickReplies =
        await this.instagramQuickReplyRepositoryService.getQuickReplyByAccountId(
          accountId,
        );
      ...
      console.error(
        `Failed to get quick replies for account: ${accountId}`,
        error,
      );
      throw error;
    }
  ...
  async createQuickReply(
    accountId: string,
    quickReplies: Array<{ category: string; title: string; content: string }>,
  ) {
    try {
  ...
  async updateQuickReply(
    accountId: string,
    quickReplyId: string,
    updateData: { category?: string; title?: string; content?: string },
  ) {
    try {
      ...
      const updatedQuickReply =
        await this.instagramQuickReplyRepositoryService.addQuickReply(items);
      return {
        success: true,
  ...
  async deleteQuickReply(accountId: string, quickReplyId: string) {
    try {
      await this.instagramQuickReplyRepositoryService.deleteQuickReply(
        quickReplyId,
      );
  ...
  async addInstagramDMFlowstate(accountId: string, flowStateData: any) {
    try {
      ...
      const triggerNode = flowStateData.flow?.nodes?.find(
        (node: any) => node.type === 'triggerNode',
      );

      flowStateData[TriggerTypes.userDM] =
        !!triggerNode?.data?.[TriggerTypes.userDM];
      
      const result =
        await this.instagramFlowstateRepositoryService.insertFlowstateDetails(
          flowStateData,
        );
      if (flowStateData.flow.isActiveAutomation) {
        await this.instagramFlowstateRepositoryService.activateFlowstate(
          flowStateData.id,
          accountId,
        );
      }
      ...
      console.error(
        `Failed to add DM flow state for account ${accountId}:`,
        error,
      );
      throw error;
    }
  ...
  async getInstagramDMFlowstates(accountId: string) {
    try {
      const flowState =
        await this.instagramFlowstateRepositoryService.getFlowstatesByAccountId(
          accountId,
        );
      ...
      console.error(
        `Failed to get DM flow state for account ${accountId}:`,
        error,
      );
      throw error;
    }
