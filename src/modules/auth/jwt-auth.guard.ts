  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (process.env.ALLOW_ALL_AUTH === 'true') {
      ...
      if (token) {
        ...
        // No token — fallback mock user for verification team
        request['user'] = {
          user: {
            sub: '104607298978669574098', // dudesagar2@gmail.com
          },
        };
        console.warn(`[AUTH BYPASS] No token. Injecting mock user: ${request['user'].user.sub}`);
      }
