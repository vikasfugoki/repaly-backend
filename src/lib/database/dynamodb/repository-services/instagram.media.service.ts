...
export class InstagramMediaRepositoryService {
  ...
  async batchGetMediaByIds(mediaIds: string[]): Promise<any[]> {
    ...
    const results: any[] = []; // Add type annotation
  ...
  async deleteAccount(accountId: string) {
    try {
      ...
      return {
        message: `Deleted ${deletePromises.length} records for accountId: ${accountId} from 'instagram_media_repository' table`,
      };
    } catch (error) {
      console.error(
        `Error deleting for accountId ${accountId} from 'instagram_media_repository' table:`,
        error,
      );
      throw new Error(
        `Failed to delete all media for ${accountId} from 'instagram_media_repository' table.`,
      );
    }
  }
