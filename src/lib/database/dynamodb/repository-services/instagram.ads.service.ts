...
export class InstagramAdsService{
    ...
    private readonly tableName = 'instagram_ad_repository';
    ...
    getAdsByAccountId(account_id: string) {
    ...
    async isAccountIdPresent(accountId: string): Promise<boolean> {
        const params = new QueryCommand({
          TableName: this.tableName,
          IndexName: 'accountId-index',
          KeyConditionExpression: 'accountId = :id',
          ExpressionAttributeValues: { ':id': accountId },
          Limit: 1, // Optimize by fetching only 1 item
        });
        ...
        try {
          ...
          console.error('Error checking accountId presence:', error);
          return false; // or rethrow depending on use case
        }
      }
    ...
    return { message: `Deleted ${deletePromises.length} records for accountId: ${accountId} from 'instagram_ads_repository' table` };
    } catch (error) {
        console.error(`Error deleting for accountId ${accountId} from 'instagram_ads_repository' table:`, error);
        throw new Error(`Failed to delete all media for ${accountId} from 'instagram_ads_repository' table.`);
        } 
    }

    async updateAdsDetails(adsDetails: Record<string, any>) {
        try {
    
          const { id, ...updateFields } = adsDetails; // Extract id and other fields separately
      
          if (!adsDetails.id) {
              throw new Error('id is required to insert or update ads details in Conversation table');
            }
      ...
      //       ReturnValues: ``, // Correct ReturnValues value
          ...
          console.log(`Ad details updated in Ads table:`, result);  // Ensure result has Attributes
          return { success: true, message: 'Ad details updated successfully in Ads Table' };
        } catch (error) {
          console.error(`Error inserting Ad details in Ads table:`, error);
          throw new Error('Failed to insert ad details in Ads table');
        }
      }
