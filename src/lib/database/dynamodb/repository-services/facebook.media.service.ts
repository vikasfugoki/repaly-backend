...
/**
 * Repository for Facebook Page posts.
 * Mirrors InstagramMediaRepositoryService but persists to `facebook_media_repository`.
 * A media item is keyed by `id` (the Facebook post id) and carries an `accountId`
 * attribute (the owning Facebook Page id) indexed by `accountId-index`.
 */
@Injectable()
export class FacebookMediaRepositoryService {
  private readonly tableName = 'facebook_media_repository';
  ...
  async batchGetMediaByIds(mediaIds: string[]): Promise<any[]> {
    ...
    for (let i = 0; i < mediaIds.length; i += BATCH_SIZE) {
      const chunk = mediaIds.slice(i, i + BATCH_SIZE);
  ...
  async insertMultipleMediaDetails(mediaDetailsList: Record<string, any>[]) {
    ...
    try {
      ...
      await this.dynamoDbService.dynamoDBDocumentClient.send(params);
      console.log(
        `Inserted ${mediaDetailsList.length} media items successfully`,
      );
      return {
        success: true,
        message: 'Media details batch stored successfully',
      };
    } catch (error) {
  ...
  async updateMediaDetails(mediaDetails: Record<string, any>) {
    try {
      ...
      const result = await this.dynamoDbService.dynamoDBDocumentClient.send(
        new UpdateCommand(params),
      );

      console.log(`Media details updated:`, result);
  ...
  async deleteAccount(accountId: string) {
    try {
      ...
      return {
        message: `Deleted ${deletePromises.length} records for accountId: ${accountId} from 'facebook_media_repository' table`,
      };
    } catch (error) {
      console.error(
        `Error deleting for accountId ${accountId} from 'facebook_media_repository' table:`,
        error,
      );
      throw new Error(
        `Failed to delete all media for ${accountId} from 'facebook_media_repository' table.`,
      );
    }
  }
  ...
  async getMediaIdsByAccountId(accountId: string): Promise<string[]> {
    try {
      ...
      console.error(
        `Error fetching media IDs for accountId ${accountId}:`,
        error,
      );
